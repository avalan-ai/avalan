...
Avalan supports text, vision, and audio workloads. See the
[modalities guide](docs/MODALITIES.md) for CLI and Python examples for each
supported modality.
...
> [!NOTE]
> This example needs the `code` extra in addition to `tool`, for example
> `python3 -m pip install -U "avalan[agent,tool,code]"`.
...
> [!NOTE]
> Browser examples need the `browser` extra in addition to `tool`, for example
> `python3 -m pip install -U "avalan[agent,tool,browser]"`.
...
> [!NOTE]
> This ReACT example uses `browser.open`, so install the `browser` extra
> alongside `tool`, for example
> `python3 -m pip install -U "avalan[agent,tool,browser]"`.
