...
def _repository(tmp_path: Path) -> Path:
    ...
    for relative, content in (
        ...
        (
            "tests/fixtures/conversation/acceptance_manifest.phase3.json",
            '{"current_phase":3}\n',
        ),
...
def test_phase_three_owns_database_across_entire_mirrored_gate(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _initialize_git(root)
    manifest = (
        root / "tests/fixtures/conversation/acceptance_manifest.phase3.json"
    )
    ...
    monkeypatch.setattr(_RUNNER, "run", run_command)
    monkeypatch.setattr(_RUNNER, "_owned_postgresql_database", owned_database)
    monkeypatch.setattr(_RUNNER, "_CONVERSATION_CURRENT_PHASE", 3)

    assert _RUNNER.run_gate(3, repo_root=root) == 0
    assert len(calls) == 6
    assert events == [
        "database-enter",
        "run-1",
...
@pytest.mark.parametrize("through_phase", (-1, 4))
def test_runner_rejects_unimplemented_phase_before_cleanup_or_coverage(
    ...
    with pytest.raises(_RUNNER.ContractGateError, match="range 0..3"):
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("test-conversation-pgsql-exact", "4"),
    ),
)
