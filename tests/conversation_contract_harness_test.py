...
def _repository(tmp_path: Path) -> Path:
    ...
    for relative, content in (
        ...
        (
            "tests/fixtures/conversation/acceptance_manifest.phase8.json",
            '{"current_phase":8}\n',
        ),
...
@pytest.mark.parametrize("through_phase", (-1, 9))
def test_runner_rejects_unimplemented_phase_before_cleanup_or_coverage(
    ...
    with pytest.raises(_RUNNER.ContractGateError, match="range 0..8"):
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("test-conversation-pgsql-exact", "9"),
    ),
)
