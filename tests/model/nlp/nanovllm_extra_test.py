from avalan.model.nlp.text.nanovllm import NanoVllmModel, NanoVllmStream
...
class NanoVllmStreamTestCase(IsolatedAsyncioTestCase):
    async def test_constructor_and_anext(self):
        iterator = iter(["a", "b"])
        stream = NanoVllmStream(iterator)
...
class NanoVllmModelTestCase(IsolatedAsyncioTestCase):
    model_id = "test-model"
    ...
    def _make_model(self):
        ...
        return NanoVllmModel(self.model_id, settings)
    ...
    def test_constructor_loads_model_and_tokenizer(self):
        ...
        nano_mock.SamplingParams = MagicMock()
        ...
        with patch.dict("sys.modules", {"nanovllm": nano_mock}):
            with (
                ...
                patch("avalan.model.nlp.text.nanovllm.LLM", nano_mock.LLM),
            ):
                settings = TransformerEngineSettings()
                model = NanoVllmModel(self.model_id, settings)
    ...
    def test_load_model_without_vllm(self):
        model = self._make_model()
        with patch("avalan.model.nlp.text.nanovllm.LLM", None):
    ...
    def test_load_model_with_vllm(self):
        ...
        with patch("avalan.model.nlp.text.nanovllm.LLM", llm_mock):
    ...
    def test_build_sampling_params_without_vllm(self):
        model = self._make_model()
        with patch("avalan.model.nlp.text.nanovllm.SamplingParams", None):
            with self.assertRaises(AssertionError):
    ...
    def test_build_sampling_params_with_vllm(self):
        ...
        with patch("avalan.model.nlp.text.nanovllm.SamplingParams", sp_mock):
