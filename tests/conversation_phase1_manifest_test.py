...
def test_phase11_defaults_select_the_complete_snapshot_family() -> None:
    """Select Phase 11 acceptance, type, failure, and threat snapshots."""
    type_verifier = _load(
    ...
    assert acceptance_path.name == "acceptance_manifest.phase11.json"
    assert (
    ...
    assert acceptance.load_manifest(acceptance_path).current_phase == 11
    assert (
    ...
    assert runner._CONVERSATION_CURRENT_PHASE == 11
    runner._validate_through_phase(_ROOT, 11)
    with pytest.raises(runner.ContractGateError):
        runner._validate_through_phase(_ROOT, 12)
