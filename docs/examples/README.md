...
- [agent_shell_images.toml](agent_shell_images.toml) – Agent with opt-in `shell.montage` and `shell.view_image` for genuine model-visible image attachments. Use a provider that supports tool-result images and keep the workspace/image bounds narrow.
