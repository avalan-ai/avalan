...
except Exception:  # pragma: no cover - nano-vllm may not be installed
    LLM = None
    SamplingParams = None


class NanoVllmStream(TextGenerationVendorStream):
...
class NanoVllmModel(TextGenerationModel):
    def __init__(
        self,
        model_id: str,
    ...
    def _load_model(self):
        assert LLM, "nano-vllm is not available"
        return LLM(
    ...
    def _build_sampling_params(
        ...
        assert SamplingParams, "nano-vllm is not available"
    ...
    async def _stream_generator(
        ...
        stream = NanoVllmStream(iter(iterator))
