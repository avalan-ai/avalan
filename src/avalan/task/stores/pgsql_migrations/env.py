...
def _prepare_connection(connection: Any) -> None:
    lock_id = int(
        config.get_main_option("task_advisory_lock_id")
        or "8172673911930301927"
    )
    schema = _task_schema()
...
def _task_schema() -> str | None:
    schema = config.attributes.get("task_schema")
    if schema is None:
        schema = config.get_main_option("task_schema")
    return str(schema) if schema else None


def _version_table() -> str:
    version_table = config.get_main_option("version_table")
    return version_table or "avalan_task_alembic_version"
