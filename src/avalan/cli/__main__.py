...
class CLI:
    ...
    def __init__(self, logger: Logger):
        ...
        default_locales_path = join(Path(
    ...
    def _create_parser(
        ...
        global_parser.add_argument(
            ...
            help="If specified, no welcome screen and only model output is "
                 "displayed in model run (sets " + ", ".join([
                "--disable-loading-progress-bar",
        ...
        model_options_parser.add_argument(
            ...
            type=str,
            action="append",
            help="Special token to add to tokenizer, only when model "
                 "is loaded",
        )
        model_options_parser.add_argument(
            "--token",
        ...
        agent_message_parser = agent_command_parsers.add_parser(
            name="message", description="Manage AI agent messages",
            parents=[global_parser]
        )
        ...
        agent_run_parser = agent_command_parsers.add_parser(
            name="run", description="Run an AI agent",
        ...
        agent_run_parser.add_argument(
            "--id",
            type=str,
            help="Use given ID as the agent ID"
        )
        ...
        agent_serve_parser = agent_command_parsers.add_parser(
            name="serve", description="Serve an AI agent as an API endpoint",
            parents=[global_parser]
        )
        ...
        cache_list_parser = cache_command_parsers.add_parser(
            name="list", description="List cache contents",
            parents=[global_parser]
        )
        ...
        flow_parser = command_parsers.add_parser(
            name="flow",
            description="Manage AI flows",
            parents=[global_parser]
        )
        flow_command_parsers = flow_parser.add_subparsers(
            dest="flow_command"
        )
        ...
        model_display_parser = model_command_parsers.add_parser(
            name="display", description="Show information about a model",
        ...
        model_command_parsers.add_parser(
            name="install", description="Install a model",
            parents=[global_parser, model_options_parser]
        )
        ...
        model_run_parser.add_argument(
            ...
            help="Attention implementation to use (defaults to best "
                 "available)",
        )
        model_run_parser.add_argument(
            "--do-sample",
        ...
        model_search_parser = model_command_parsers.add_parser(
            name="search", description="Search for models",
            parents=[global_parser]
        )
        ...
        model_uninstall_parser = model_command_parsers.add_parser(
            name="uninstall", description="Uninstall a model",
            parents=[global_parser, model_options_parser]
        )
        ...
        tokenizer_parser = command_parsers.add_parser(
            name="tokenizer",
            description="Manage tokenizers, loading, modifying and "
                        "saving them",
            parents=[global_parser]
        )
    ...
    async def _main(
        ...
        log_level = DEBUG if verbosity >= 2 else \
                    INFO if verbosity >= 1 else \
                    WARNING
        ...
        if args.login or (suggest_login and Confirm.ask(
            theme.ask_login_to_hub(),
            default=False
        )):
            with console.status(
