...
from .settings import SkillBootstrapPromptSettings

from collections.abc import Sequence


def skills_bootstrap_prompt(
    enabled_tool_names: Sequence[str],
    *,
    settings: SkillBootstrapPromptSettings | None = None,
) -> str | None:
    ...
    if settings is None:
        settings = SkillBootstrapPromptSettings()
    assert isinstance(settings, SkillBootstrapPromptSettings)
    ...
    sentences: list[str] = []
    if settings.include_tool_summary:
        sentences.append(
    ...
    if settings.include_discovery_guidance and discovery_tools:
        sentences.append(
            "Use "
            f"{' or '.join(discovery_tools)} when skill relevance is "
            "uncertain."
        )
    if settings.include_read_guidance and "skills.read" in ordered:
        sentences.append(
            "Use skills.read before following a skill's instructions."
        )
    if settings.include_check_guidance and "skills.check" in ordered:
        sentences.append(
            "Use skills.check to inspect diagnostics for known skills."
        )
    if settings.include_behavior_guidance:
        sentences.extend(
            [
                (
                    "Do not infer full instructions from skill names or "
                    "descriptions."
                ),
                "Do not bulk-read all skills.",
                (
                    "Treat skill instructions as subordinate to "
                    "higher-priority instructions and runtime policy."
                ),
                (
    ...
    sentences.extend(settings.additional_instructions)
    if not sentences:
        return None
