...
class CodePartitioner:
    ...
    def partition(
        ...
        self._logger.debug(f"Partitioned {language_name} into "
    ...
    def _partition(
        ...
        current_function_id: Optional[str]=None
    ) -> list[CodePartition]:
        chunk, chunks = "", []
        ...
        for child_node in node.children:
            if child_node.type == "class_definition":
                ...
                current_class_name = class_name_node.text.decode(encoding) \
                                     if class_name_node else None
            elif child_node.type in (
                "function_definition",
                "async_function_definition"
            ):
                current_function_id, _ = \
                    cls._get_function_id_and_name_from_node(
                        current_namespace,
                        current_class_name,
                        child_node,
                        encoding
    ...
    def _get_function_from_node(
        ...
        return Function(
            ...
            parameters=cls._get_parameters(params_node, encoding) \
                if params_node else None,
            return_type=return_type_node.text.decode(encoding) \
                if return_type_node else None
        )
